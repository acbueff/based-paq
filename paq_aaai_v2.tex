...
\begin{abstract}
Real document-VQA workloads are multi-query: one long document is prefilled once and asked many different questions, yet standard KV-cache eviction commits to one compressed context for all questions. We present \paqc{} (``prefill once, ask many''), a training-free architecture for per-query re-selection over a retained full-context KV cache: prefill once, score 64-token page-aligned blocks with the reader's own question-to-content attention, and decode under a restricted-attention 4D mask over the intact cache, with no gathering, no re-reading, no second model, and no index. Because nothing is discarded, selection is fully reversible, which is the property the architecture is built to provide: per-query re-selection outperforms the identical scorer committed once, a strong gold-informed static subset, and a trained retriever's committed form (isolation contrast $+0.264$, $\lb=+0.187$ under a clean-cache re-run; block-level replication $+0.379$, $\lb=+0.273$). At a nominal 5\% (realized ${\approx}6\%$) decode-attention budget, \paqc{} outperforms ColQwen2, a state-of-the-art trained visual retriever, on MMLongBench-Doc ($0.4042$ vs.\ $0.3282$ with ColQwen2's trained retrieval-projection LoRA correctly loaded; $+0.077$, document-clustered $\lb=+0.040$; 80 documents, 444 decoded / 440 paired), matches full context, and is not statistically distinguishable from the gold-page oracle at its ${\sim}6\%$ budget ($-0.026$; the CI straddles zero, so this is parity within noise rather than demonstrated equality). On LongDocURL \paqc{} matches but does not outperform the retriever ($+0.019$, $\lb=-0.019$; below full context by $-0.014$); we report this as a limitation and observe that the cell appears largely reader-conversion-bound rather than selection-bound. Two selector upgrades (a per-head mixture transfer gate that stopped a planned LoRA extension before any adapter was trained, and a sharp expert-head scorer) failed their pre-registered gates and are reported as negative results. The improvement over the retriever is confirmed out-of-sample: on a fresh, disjoint, sealed 45-document / 244-query MMLongBench-Doc set evaluated exactly once under a pre-registered frozen harness, \paqc{}@5\% scores $0.396$ vs.\ ColQwen2 $0.320$ ($+0.076$, document-clustered $\lb=+0.029$; $244/244$ complete, robust to worst-case imputation).
\jiahui{too long, first backgroundn and challenges, then proposed approach, and finally 1-2 sentences mention the digits.}
...
\caption{Restricted-attention mechanism. \textbf{A:} The retained cache keeps scaffold, page content in 64-token page-aligned blocks, question, and generated tokens at their original positions. \textbf{B:} For each decode row, the 4D additive mask exposes only selected blocks and the always-keep set; masked blocks remain available to later questions. \textbf{C:} Decode attends to that view without gathering or evicting K/V, so M-RoPE indices remain untouched. An all-true mask reproduced native decode in every audited case (zero mask-specific argmax changes).}
\label{fig:paqkvmask}
\end{figure*}
...
\jiahui{Figures are not native Python drawn, better not use such AI style.}
\begin{figure*}[t]
\centering
\includegraphics[width=0.92\textwidth]{fig_j_reversibility_light.pdf}
\caption{Reversibility, the tested axis. \textbf{Top:} commit-once reuses one fixed subset across questions, whereas re-selection derives a question-specific view from the same intact prefill. \textbf{Bottom:} document-clustered two-sided 95\% intervals for the confirmatory page-level contrasts, the clean-cache re-run, and the below-page \paqc{} replication. Every interval excludes zero.}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_b_isolation_forest_light.pdf}
