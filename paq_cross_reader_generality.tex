...
Qwen2.5-VL-7B (same Qwen-VL family) the method is a \emph{statistical dead heat} with the comparator
($\Delta=+0.007$, doc-cluster $\lb=[-0.037,+0.049]$); on InternVL3.5-8B (a different VLM whose language backbone is
Qwen3) it \emph{significantly underperforms} ($\lb=[-0.106,-0.029]$). Together these characterize a \textbf{mechanistic
boundary}: the method's benefit tracks how good the host reader's own attention is as a below-page retriever. We are
extending the study to four further readers (Qwen3-VL-4B, Molmo-7B-D, MiniCPM-V-4.5, PaliGemma-2). This is an internal
companion note kept separate from the submission; the shared team decides what, if anything, to fold in.
\end{abstract}
...
\begin{table*}[t]
\centering
\caption{\paqc{} vs.\ the ColQwen2-page comparator, within each reader, on the 80-doc / 444-q DEV pin. $\Delta$ and
$\lb$ are doc-cluster (paqkv $-$ comparator). Qwen3-VL-8B is the reader the method was developed and sealed on.}
\label{tab:main}
\small
\begin{tabular}{@{}lccccc@{}}
\toprule
Reader & Budget & \paqc{} & ColQwen2 & Full & doc-cluster $\lb$ (paqkv $-$ ColQwen2) \\
\midrule
Qwen3-VL-8B \textit{(native)} & $\sim$64K & \textbf{0.4042} & 0.3284 & 0.3965 & $+0.041$ \\
Qwen2.5-VL-7B                 & $\sim$52K & 0.2698 & 0.2623 & 0.3237 & $[-0.037,+0.049]$ \\
InternVL3.5-8B                & $\sim$36K & 0.1876 & 0.2542 & 0.2637 & $[-0.106,-0.029]$ \\
\bottomrule
\end{tabular}
\end{table*}
...
Table~\ref{tab:additional} records the four further readers requested for this study, with the verified architecture
facts that determine feasibility (\S\ref{sec:requires}) and the current run status. Feasibility is decided cheaply by a
Phase-0 identity spike before any full run; full ports proceed only for spike-passers, and a reader that fails the
identity gate is reported honestly as \textsc{incompatible} rather than forced.
...
\toprule
Reader & LM backbone & Vision integration & Impl. & Expected & Status \\
\midrule
Qwen3-VL-4B   & Qwen3-VL & M-RoPE, in-line tokens    & native & win-ish (size abl.)   & \pending{} \\
Molmo-7B-D    & Qwen2-7B & CLIP, in-line tokens      & remote & tie-ish (Qwen2 fam.)  & \pending{} \\
MiniCPM-V-4.5 & Qwen3-8B & 3D-Resampler (64 tok)     & remote & degenerate/neg.       & \pending{} \\
PaliGemma-2   & Gemma-2  & prefix-LM (bidir.)        & native & incompat./weak        & \pending{} \\
\bottomrule
\end{tabular}
\end{table*}
...
\item \textbf{Molmo-7B-D} uses a Qwen2 language backbone with LLaVA-style in-line visual tokens (genuinely different
training data, AI2 PixMo), but ships as remote code; whether its custom attention honors the 4-D mask
and exposes 4-D eager weights is the spike's make-or-break. If it ports, it is another Qwen2-family data point
(cf.\ Qwen2.5-VL).
\item \textbf{MiniCPM-V-4.5} has a Qwen3 backbone (the same LM family as InternVL, which was negative) and a
3D-Resampler that compresses each image to $\approx$64 tokens; at $\approx$64 visual tokens per page, a 5\% block
budget leaves almost nothing to select below page granularity, so even a clean port is expected to be near-degenerate.
It also ships as remote code.
\item \textbf{PaliGemma-2} uses a prefix-LM (bidirectional attention over the image+prefix, causal only on the suffix),
which conflicts with the method's causal restricted-decode over a prefilled cache; it is also short-output-tuned and
weakly suited to long multi-page VQA. The spike will decide between \textsc{incompatible} and a genuine-but-weak result.
\end{itemize}
...
well at 5\% and even denoises above full context); Qwen2.5-VL-7B partially has it (break-even; recovers 0.70 of
answerable queries); InternVL3.5-8B lacks it here (recovers 0.53 and loses). This \emph{confirms} rather than refutes
the single-reader observation, and reframes it from an unexamined weakness into an investigated boundary condition:
\paqc{} is a white-box method whose benefit is contingent on the host reader's attention being an effective below-page
retriever.

\begin{table*}[t]
\centering
\caption{The generality gradient. doc-cluster $\lb$(paqkv $-$ comparator); Qwen3-VL-8B shows the sealed out-of-sample
value, the second readers the DEV-pin value.}
\label{tab:gradient}
\small
\begin{tabular}{@{}llc@{}}
\toprule
Reader & Relation to the sealed reader & doc-cluster $\lb$ \\
\midrule
Qwen3-VL-8B    & native (sealed reader)    & $+0.029$ \textit{(win)} \\
Qwen2.5-VL-7B  & same family, prior gen.   & $[-0.037,+0.049]$ \textit{(tie)} \\
InternVL3.5-8B & cross-family VLM (Qwen3 LM)& $[-0.106,-0.029]$ \textit{(neg.)} \\
\bottomrule
\end{tabular}
\end{table*}
