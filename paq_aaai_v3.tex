...
Algorithm~S1 (Suppl.~\S A.1)\footnote{References of the form
Suppl.~\S X, Table~SX, Figure~SX, and Algorithm~SX point to the separate
supplementary-material document.} gives the loop
...
(Suppl.~\S C.2 gives the audit detail). Because nothing is
...
present in one prefill; Suppl.~\S A.1 gives the
...
bars (\S\ref{sec:results-scope}; Suppl.~\S B.5
and~\S B.8).
...
The executed panel (Suppl.~\S B.7) compares, per query at matched
...
rather than assumed away; Suppl.~\S A.3 gives both accounts in
...
is the historical page-level panel of Suppl.~\S B.7
...
frozen, hash-guarded harness (Suppl.~\S A.2). That one sealed
...
(Figure~\ref{fig:forest}a; full table in Suppl.~\S B.3).
...
checks (Suppl.~\S C.2): re-running the statistics on only
...
document-clustered 95\% CI excludes zero (Suppl.~\S B.1),
...
Suppl.~\S B.2 gives realized budgets, text channels, and
cohort notes.}
\label{tab:paqkv}
\end{table*}
...
(Figure~\ref{fig:forest}b; Table~S2 in
Suppl.~\S B.1). ColQwen2 here runs with its trained
...
cell. Suppl.~\S C.3 gives both recall notions and the full
...
run (Suppl.~\S A.2), \paqc{}@5\% scores $0.396$ versus
...
the same document genres (Suppl.~\S A.2).
...
Suppl.~\S C.1 details these screen analyses and their
caveats, including a screen-level budget-monotonicity failure that the
full-pin sweep of Suppl.~\S B.6 attributes to small-sample
noise.
...
and $+0.042$ $[-0.001,+0.092]$ on LDU. A BM25 page retriever trails
...
Suppl.~\S B.6; the query-blind eviction family remains in
Suppl.~\S B.7, at or below random page selection. Two
pre-registered extensions of the scorer failed their pre-frozen gates and are
reported in Suppl.~\S C.4: a learned per-head attention
...
585-query development-grade cell (Suppl.~\S A.2 details the audit
...
(Suppl.~\S A.2) confirms the capped cell did not manufacture the
result.
...
fresh-documents pin of Suppl.~\S B.8 (80 documents, 538
...
Suppl.~\S B.4). Qwen3-VL-4B largely preserves the
...
(Suppl.~\S B.5). Three caveats attach: these are development-pin
...
incompatible rather than forced (Suppl.~\S B.4).
