...
% S2=pagelevel-bars; Algorithm S1. [SUPERSEDED by the 2026-07-29 page-limit
% pass below: +S10=systems, +fig S1=sealed-bar (others shift +1), +C.6.]
...
% 2026-07-29 (page-limit compliance pass): AAAI-27 allows SEVEN pages of
% non-reference content (pp. 8-9 references ONLY); the previous build had
% ~1.7 columns of content on p.8. Fixes, all numbers unchanged: (1) Table 2
% (tab:systems) + full caption MOVED to supplement C.6 (= Table S10; placed
% at END of section C so S1-S9 and all A/B/C/D pointers keep their numbers);
% \S5.4 compressed to a summary + S10 pointer. (2) fig:sealed (sealed bar)
% MOVED to supplement A.2 (= Figure S1; old figs shift S1->S2, S2->S3 --
% no static figure-number pointer existed); \S5.2 + Table 1 caption now cite
% Figure 3b (+ Suppl. Fig. S1). (3) Prose compressions, each backed by
% verbatim supplement/duplicate text: intro M-RoPE + block-granularity
% sentences, \S3.1 index sentence, \S3.3 reversibility recap, Fig 2 caption,
% \S4.4 integrity wording, \S5.1 robustness-check numbers (-> C.2),
% \S5.2 re-slice CIs (-> C.3) + oracle/full CIs (-> Table S2) + audit
% sentence, \S5.3 MMDocRAG/MMDocIR-QA/cross-reader tightenings, \S5.4 and
% Conclusion shortened ("beats" avoided per register). RESULT: content ends
% on p.7; p.8 begins with References; 9 pages total. COMPLIANCE TEST:
% pdftotext p.8 must start with "References".
% Supplement tables S1=pins S2=contrasts S3=isolation S4=crossreader S5=xrb
% S6=budget-sweep S7=panel S8=mmdocir S9=anatomy-strata S10=systems;
% figs S1=sealed-bar S2=reversibility S3=pagelevel-bars; Algorithm S1.
% 2026-07-29 (later, human GO): Table 1 gains an "MMLB (sealed)" column
% (0.3957/0.3202, verified against paq_sealed_final_sealed_verdict.json
% means_complete_case; 45 docs / 244 q) next to MMLB; all arms not run on
% the sealed set are "--"; caption's "deliberately not a column" sentence
% replaced by the sealed-column explanation. Numbers match the \S5.2 prose
% (0.396/0.320 at 3 dp) and Supplementary Table S2's sealed row.
...
fine enough to isolate sub-page evidence yet coarse enough for their scores
to be stable. After the
...
discarded, every selection is furthermore \emph{reversible}, the axis our
evaluation isolates (\S\ref{sec:setup-stats},
...
gate (Supplementary~\S A.2). The gate is executed with \paqc{} itself as the fresh
...
\toprule
 & LDU & MMLB & MMLB (sealed) & MMDocRAG & MMDocIR-QA \\
 & \scriptsize 80 docs / 463 q & \scriptsize 80 docs / 444 q & \scriptsize 45 docs / 244 q & \scriptsize 62 docs / 585 q & \scriptsize 80 docs / 538 q \\
\midrule
...
% MMLB (sealed) column (added 2026-07-29, human GO): research/paq_sealed_final_sealed_verdict.json
% (means_complete_case paqkv 0.3957 / rf 0.3202; 45 docs, 244/244 complete); only the
% paqkv-vs-ColQwen2 comparison was run on the sealed set, all other arms are "--".
...
\paqc{}@5\% (\textbf{ours}) & \textbf{0.5315} & \textbf{0.4042}$^{*}$ & \textbf{0.3957}$^{*}$ & \textbf{0.4089}$^{*}$ & 0.3654 \\
ColQwen2-fresh (matched) & 0.5121 & 0.3282 & 0.3202 & 0.3517 & \textbf{0.4080}$^{*}$ \\
\midrule
...
development-grade cells with single runs and no pre-registered bars. MMLB
(sealed) is the out-of-sample confirmation, a fresh disjoint set evaluated
exactly once under the frozen harness (\S\ref{sec:integrity}); only the
\paqc{}-versus-ColQwen2 comparison ran there (other arms --;
Figure~\ref{fig:forest}b plots the contrast). All baseline rows share
...
development-pin only. This single evaluation is the strongest evidence in
the paper and directly addresses the development pin's reuse. On the
development pin, \paqc{} at the same budget is indistinguishable from full
context ($+0.009$) and not statistically distinguishable from the gold-page
oracle ($-0.026$); the latter interval permits material inferiority, so
this is parity within noise rather than demonstrated equality
(Supplementary Table~S2). A training-distribution audit found no
...
cross-page evidence). MMDocRAG is corpus-adjacent to our pins, so every
document overlapping our development or sealed pins was excluded before any
experiment, leaving a fresh 62-document, 585-query development-grade cell
(Supplementary~\S A.2 details the audit and protocol deviations). The improvement over the trained
...
MMDocIR's questions carry reference answers as well as gold evidence, so its
fresh-documents pin (80 documents, 538 queries, same overlap audit;
Supplementary~\S B.8) yields a fourth, development-grade QA cell, the most
retrieval-native of the four: its questions were written per annotated
layout for retrieval evaluation. Here the trained retriever wins.
...
Qwen3-VL-8B, we ported the complete method to three further VLMs
(Supplementary~\S B.4): Qwen3-VL-4B (the same
...
Qwen3). Every port passed the all-true-mask decode-identity gate before any
accuracy number was read. The outcome is a gradient ordered by
...
read the boundary as architectural rather than a matter of scale: \paqc{}
is a white-box method whose benefit is contingent on the host reader's
attention being an effective below-page retriever.
\paqc{} nonetheless stays ahead of every training-free baseline
on every reader, including where it loses to the trained retriever
...
configuration; Supplementary Table~S10), \paqc{} is roughly $2.25\times$ slower per
...
roughly $11\times$ faster than re-reading the document per question, and
the prefill amortizes after about one query per document. The
...
index, no repeated prefills.

\section{Conclusion}
\paqc{} turns a frozen VLM's own KV cache into a queryable, reversible
evidence store: prefill once, then re-select and masked-decode per query
over the intact cache. Per-query re-selection outperforms committing the
...
evaluated exactly once. The advantage reproduces where evidence is
dispersed, ties where conversion binds, reverses on retrieval-native
questions, and requires a host reader whose own attention is an effective
below-page retriever; that scope law is the paper's hypothesis.
