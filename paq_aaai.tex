...
\paragraph{Evidence status.} Every accuracy number in this paper is a development-pin result on an 80-document-per-cell pin, \emph{except} the out-of-sample confirmation of \S\ref{sec:results-mmlo}: the MMLongBench-Doc beat was evaluated \emph{once} on a fresh, disjoint, sealed 45-document / 244-query set and holds ($+0.076$, $\lb=+0.029$). The sealed LongDocURL documents remain unevaluated (out of scope for this MMLongBench-scoped result). We state the evidence status wherever it binds.
